...
void conv_optimized(const float* in, float* out, const float* ker,
                  int H, int W, int K) {
    return conv_optimized_8(in, out, ker, H, W, K);
}
...
// Cache tiling + SIMD only. Unlike conv_optimized_8_tiled, this version processes
// one vector at a time, so it provides a clean comparison without loop unrolling.
void conv_optimized_tiled_simd(const float* in, float* out, const float* ker,
    ...
    for (int ty = 0; ty < H; ty += Ty) {
        const int oy_end = (ty + Ty < H) ? ty + Ty : H;

        for (int tx = 0; tx < W; tx += Tx) {
            const int ox_end = (tx + Tx < W) ? tx + Tx : W;

            for (int oy = ty; oy < oy_end; ++oy) {
                float* out_row = out + oy * W;
                int ox = tx;

                // One AVX-512 vector produces 16 adjacent output pixels.
                for (; ox <= ox_end - 16; ox += 16) {
                    __m512 acc = _mm512_setzero_ps();
                ...
                // Process a possible eight-float remainder within the tile.
                if (ox <= ox_end - 8) {
                    __m256 acc = _mm256_setzero_ps();
                ...
                // Process a possible four-float remainder within the tile.
                if (ox <= ox_end - 4) {
                    __m128 acc = _mm_setzero_ps();
                ...
                // Scalar cleanup handles widths that are not multiples of four.
                for (; ox < ox_end; ++ox) {
                    float acc = 0.0f;
...
void conv_optimized_8_tiled(const float* in, float* out, const float* ker,
                          int H, int full_W, int K) {
    const int p = K / 2;
    const int in_stride = full_W + 2 * p;
    const int fpi = 16;
    const int unroll = 8;
    const int Tx = 256;
    const int Ty = 16;
    ...
    for (int ty = 0; ty < H; ty += Ty) {
        const int oy_end = (ty + Ty < H) ? ty + Ty : H;
        for (int tx = 0; tx < full_W; tx += Tx) {
            const int ox_end = (tx + Tx < full_W) ? tx + Tx : full_W;
            for (int oy = ty; oy < oy_end; ++oy) {
                float* out_row = out + oy * full_W;
                int ox = tx;
                const int W = ox_end;
